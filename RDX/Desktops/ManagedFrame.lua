...
local function Default_Drag(frame)
	if (frame._dk_nolayout) or (frame._dk_drag) then return; end
	...
	frame._dk_drag = RDXDK.GetFrame(dragframeprops.name)
...
function RDXDK.SavePosition(frame, frameprops, save)
	if frame then 
	...
	--if save then RDXDK._WriteFrameProps(frameprops); end
	if save then RDXDK._SaveFrameProps(true, frameprops); end
end