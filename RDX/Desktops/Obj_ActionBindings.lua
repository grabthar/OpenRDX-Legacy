...
RDXAB = RegisterVFLModule({
	name = "RDXAB";
	title = i18n("RDX Action Bindings Object");
...
function RDXAB.RegisterActionType(tbl)
	if (not tbl) or (not tbl.name) then RDX.printW(i18n("attempt to register anonymous Action type bindings")); return; end
	local n = tbl.name;
	if actiontypes[n] then RDX.printW(i18n("Duplicate registration Action type bindings ") .. tbl.name); return; end
	actiontypes[n] = tbl;
end
...
local function saveActionBindings(actionslist)
	for i = 1, 120 do
		...
		elseif actiontype == "spell" then
			actionid = RDXSS.GetSpellFullName(actioninfo);
...
local function loadActionBindings(actionslist)
	for i = 1, 120 do
		...
		if not newaction or actiontype ~= newaction.actiontype or actionid ~= newaction.actionid then
			if newaction.actiontype == "companion" then 
			VFL.print(actiontype);
			VFL.print(actioninfo);
			VFL.print(subType);
			VFL.print(globalID); 
...
RDXDB.RegisterObjectType({
	...
	Instantiate = function(path, obj)
		if currentActionBindings then RDX.print("a action bindings is already instantiated"); return nil; end
