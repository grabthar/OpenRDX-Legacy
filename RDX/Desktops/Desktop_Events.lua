...
local function GenerateEventDispatchUI(desc, ui, txt)
	local devent = RDXUI.CreateElementEdit(ui, txt, RDXDK._GetEventCache)
...
RDX.RegisterFeature({
	...
	ApplyFeature = function(desc, state)
		...
		WoWEvents:Bind("]]..desc.bevent..[[", nil, function()
	...
	UIFromDescriptor = function(desc, parent)
		...
		local bevent = RDXUI.CreateElementEdit(ui, i18n("Bind to event:"), msgEvents)
...
RDX.RegisterFeature({
	...
	UIFromDescriptor = function(desc, parent)
		...
		local devent, lockout, rpc = GenerateEventDispatchUI(desc, ui, i18n("If event, Dispatch Event:"))		
...
RDX.RegisterFeature({
	...
	title = i18n("DesktopEvent: RDX Set"),
	category = i18n("Events");
	multiple = true;
	IsPossible = function(state)
		if not state:Slot("Desktop") then return nil; end
	...
	ExposeFeature = function(desc, state, errs)
		...
		if not desc.set then 
			VFL.AddError(errs, i18n("You must select a RDX Set"));
			return false;
		end
		if not desc.devent or desc.devent == "" then 
			VFL.AddError(errs, i18n("You must dispatch an event for if in set"));
			return false;
		end
		if not desc.odevent or desc.odevent == "" then 
			VFL.AddError(errs, i18n("You must dispatch an event for if not in set"));
			return false;
		end
		return true;
	end;
	ApplyFeature = function(desc, state)
		...
		local set]] .. id ..[[ = RDX.FindSet(]].. Serialize(desc.set) .. [[);
		local setObject]] .. id ..[[ = {};
		local Check]] .. id ..[[ = function()
			if (set]] .. id ..[[:GetSize() > 0) then
	...
	UIFromDescriptor = function(desc, parent)
		...
		local devent = RDXUI.CreateElementEdit(ui, "If in Set Dispatch Event:", RDXDK._GetEventCache())
		if desc and desc.devent then devent.editBox:SetText(desc.devent); end
		ui:InsertFrame(devent);
		
		local odevent = RDXUI.CreateElementEdit(ui, "If not in Set Dispatch Event", RDXDK._GetEventCache())
