...
function RDXDK.FrameProperties(frame)
	...
	local mnu = {};
	table.insert(mnu, { text = i18n("Layout..."), OnClick = function() RDXDK.LayoutPropsDialog(frameprops); VFL.poptree:Release(); end });
	...
	if RDXDK.IsDocked(frameprops) then
		table.insert(mnu, {text = i18n("Undock"), OnClick = function()
				local frame = RDXDK.GetFrame(frameprops.name);
				RDXDK.ResetDockGroupLayout(frame); 
				RDXDK.CompletelyUndock(frameprops, true);
				RDXDK.LayoutAll(); 
				VFL.poptree:Release();
			end });
		if not RDXDK.IsDGP(frameprops) then
			table.insert(mnu, {text = i18n("Make Dock Parent"), OnClick = function()
				local frame = RDXDK.GetFrame(frameprops.name);
				RDXDK.MakeDockGroupParent(frameprops);
				RDXDK.ResetDockGroupLayout(frame); 
	...
	if frameprops.feature == "desktop_window" then
		table.insert(mnu, {
			...
			OnClick = function()
				VFL.poptree:Release();
				RDXDK.QueueLockdownAction(frameprops.name, RDXDK._AsyncRebuildWindowRDX);
			end
		});
		table.insert(mnu, {
			...
			OnClick = function()
				VFL.poptree:Release();
				local md = RDXDB.GetObjectData(frameprops.name);
				if md then RDX.EditWindow(frameprops.name, md); end
			end
		});
	elseif frameprops.feature == "desktop_statuswindow" then
		...
		table.insert(mnu, {
			...
			OnClick = function()
				VFL.poptree:Release();
				local md = RDXDB.GetObjectData(frameprops.name);
				if md then RDX.EditStatusWindow(frameprops.name, md); end
			end
		});
	end
	...
	table.insert(mnu, { text = i18n("Close"), OnClick = function() 
		VFL.poptree:Release();
		RDXDK.QueueLockdownAction(frame._dk_name, RDXDK._CloseWindowRDX);
	end });
...
function RDXDK.LayoutPropsDialog(frameprops)
	...
	btnOK:SetScript("OnClick", function()
		...
		local frame = RDXDK.GetFrame(dd.name);
		RDXDK.ResetDockGroupLayout(frame);
		RDXDK.LayoutDockGroup(frame);
		--RDXDK._WriteFrameProps(dd);
