...
local cbnew, cbappend, cbtostr = ConcatBuffer.new, ConcatBuffer.append, ConcatBuffer.toString;
local ZMSchedule = VFL.ZMSchedule;
...
function RPC.StdOutputStream:new()
	...
	function os:Clear()
		if locked then error(i18n("attempt to clear locked channel")); end
			...
			if VFL.isArray(obj) then
				...
				for k,v in pairs(obj) do
					if (type(k) == "string" or type(k) == "number") then
						...
						VFL.TripError("RDX", i18n("Warning: attempt to serialize table with non-scalar key."), "");
					end
				end
				self:_AppendByte(253);
			end
	...
	function os:SendLine()
		if not locked then error(i18n("attempt to dispatch from unlocked stream")); end
...
--	VFL.print("stack_vector " .. op .. "," .. tostring(a1) .. "," .. tostring(a2));
	...
	elseif(op == 3) then
		if a1 == "VECTOR" then -- No nested vectors!
			StackError(i18n("Nested vectors"));
			return; 
		end 
	...
	elseif(op == 3) then
		if data.__index ~= nil then
			...
			if a1 == "NUMBER" or a1 == "STRING" then
				...
				StackError(i18n("Table index must be STRING or NUMBER")); return;
			end
		end
	end
end
...
local function PushStack(st, a1, a2)
	...
	-- Add to the stack
	sptr = sptr + 1;
	if(sptr > 64) then StackError(i18n("Serialization stack overflow")); end
	state_stack[sptr] = st;
...
function RPC.StreamDecoder:new()
	...
	function self:Open(sid, sdr, chn, hdlr)
		if (not sid) or (not sdr) or (not chn) or (not hdlr) then
			error(i18n("invalid parameters to StreamDecoder:Open()"));
		end
		if isOpen then error(i18n("Open on already-open stream.")); end
		...
		while i <= strlen(str) do
			...
			elseif(n == 254) then -- Begin vectored object.
				...
				elseif n == 3 then
					...
					StackError(i18n("Invalid vectored object type"));
				end
...
VFL.AdaptiveSchedule(nil, 5, TimeoutTroll);
...
function RPC.StdStreamIn(channel, sender, prefix, line)
	...
	if preSeq ~= strm.preSeq then strm:Error(i18n("Out-of-sequence packet")); return; end
	strm.preSeq = postSeq;
	-- Verify the sender.
...
local function _CreateLongRPCWindow()
	...
	_lrpc_window = RDX.Window:new(RDXParent);
	_lrpc_window:LoadState(state);
	_lrpc_window.RepaintAll = state:GetSlotFunction("RepaintAll");
	_lrpc_window:WMGetPositionalFrame():SetPoint("CENTER", UIParent, "CENTER");
