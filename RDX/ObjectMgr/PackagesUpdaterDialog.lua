...
local function CellOnClick(self)
	if RDXDB.GetRAUMode() == 2 then
		if(arg1 == "RightButton") then
			local mnu = {
				{ text = strcolor(.5, .5, .5) .. "Package " .. self.col[1]:GetText() .. "|r" };
			};
			table.insert(mnu, { 
				text = "Download", 
				OnClick = function() 
			...
			VFL.poptree:Begin(120, 12, this, "TOPLEFT", GetRelativeLocalMousePosition(self));
...
local function RAU_ReqPck_Rcvd(ci, resp)
	...
	RDX.TryIntegrate(UIParent, resp, "test", nil);
...
function RDXDB.ToggleRAU()
	if RDXG.ShowRAU then
		RDXDB.HideRAU();
		--RDX.printI(i18n("Hide RAU"));
	else
		RDXDB.ShowRAU()
		--RDX.printI(i18n("Show RAU"));
	end
end
