...
local function OpenBindingWindow(parent)
	...
	binding_window:SetScript("OnKeyDown", function()
	...
	btnOK:SetText(i18n("SAVE")); btnOK:SetHeight(25); btnOK:SetWidth(80);
...
function __RDXBindingKeyOnEnter(self)
	...
	if bindingText ~= "" then
		GameTooltip:SetText(i18n("Click to change binding"));
		GameTooltip:AddLine(i18n("Right-click to remove binding"), 1, 1, 1);
		local color = GREEN_FONT_COLOR;
		GameTooltip:AddDoubleLine(i18n("Currently Bound:"), bindingText, 1, 1, 1, color.r, color.g, color.b);
	else
		GameTooltip:SetText(i18n("Click to bind a key"));
	end
	GameTooltip:Show();
end
...
function __RDXBindingKeyOnClick(self)
