...
RDX.RegisterFeature({
	...
	title = i18n("Bar and Text Timer Shader"); category = i18n("Shaders");
...
frame.]] .. objname .. [[:Destroy(); frame.]] .. objname .. [[ = nil;
	...
	UIFromDescriptor = function(desc, parent, state)
		...
		local countTypeFlag = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Count type (true CountUP, false CountDOWN)"), state, "BoolVar_", nil, "true", "false");
		if desc and desc.countTypeFlag then countTypeFlag:SetSelection(desc.countTypeFlag); end
		
		ui:InsertFrame(VFLUI.Separator:new(ui, i18n("Status Bar Timer")));
		...
		chk_sbblendcolor:SetText(i18n("Use blend color"));
		...
		local sbcolorVar1 = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Empty color variable"), state, "ColorVar_");
		if desc and desc.sbcolorVar1 and type(desc.sbcolorVar1) == "string" then sbcolorVar1:SetSelection(desc.sbcolorVar1); end
		
		local sbcolorVar2 = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Full color variable"), state, "ColorVar_");
		...
		local text = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Text Timer"), state, "TextCustom_");
		if desc and desc.text then text:SetSelection(desc.text); end
		
		local tt = RDXUI.EmbedRight(ui, i18n("Text Timer Type:"));
		local dd_textType = VFLUI.Dropdown:new(tt, RDXUI.TextTypesDropdownFunction);
		...
		chk_duration:SetText(i18n("Show max duration"));
		...
		chk_blendcolor:SetText(i18n("Use blend color"));
		...
		local color1 = RDXUI.GenerateColorSwatch(ui, i18n("Static min color"));
		if desc and desc.color1 then color1:SetColor(explodeRGBA(desc.color1)); end

		local color2 = RDXUI.GenerateColorSwatch(ui, i18n("Static max color"));
		...
		chk_stack:SetText(i18n("Use variable instead of time for blend color"));
		if desc and desc.stack then chk_stack:SetChecked(true); else chk_stack:SetChecked(); end
		ui:InsertFrame(chk_stack);
		
		local var_stackVar = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Variable number"), state, "Var_");
		...
		ed_stackMax:SetText(i18n("Variable number max "));
		...
		local textInfo = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Text Info"), state, "TextCustom_");
		if desc and desc.textInfo then textInfo:SetSelection(desc.textInfo); end
		
		local txt = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Text Info data"), state, "TextData_");
		if desc and desc.txt then txt:SetSelection(desc.txt); end
		
		ui:InsertFrame(VFLUI.Separator:new(ui, i18n("Icon texture")));
		
		local texIcon = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Texture Icon"), state, "Texture_");
		if desc and desc.texIcon then texIcon:SetSelection(desc.texIcon); end

		local tex = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Texture Icon data"), state, "TexVar_");
		if desc and desc.tex then tex:SetSelection(desc.tex); end
		
		----------------------------------------------------------------------------
		ui:InsertFrame(VFLUI.Separator:new(ui, i18n("Shader")));
		
		local chk_defaultTL = VFLUI.Checkbox:new(ui); chk_defaultTL:Show();
		chk_defaultTL:SetText(i18n("Use Timeleft Condition"));
		...
		ed_TL:SetText(i18n("Show when Timeleft is inferior"));
		if desc and desc.TL then ed_TL.editBox:SetText(desc.TL); else ed_TL.editBox:SetText("0");end
		ui:InsertFrame(ed_TL);
		
		local er_sbDefaultState = RDXUI.EmbedRight(ui, i18n("Status Bar Default State:"));
		...
		local er_sbExpireState = RDXUI.EmbedRight(ui, i18n("Status Bar Shader State:"));
		...
		local er_TTIDefaultState = RDXUI.EmbedRight(ui, i18n("Text Info Default State:"));
		...
		local er_TTIExpireState = RDXUI.EmbedRight(ui, i18n("Text Info Shader State:"));
		...
		local er_TEIDefaultState = RDXUI.EmbedRight(ui, i18n("Icon Texture Default State:"));
		...
		local er_TEIExpireState = RDXUI.EmbedRight(ui, i18n("Icon Texture Shader State:"));
