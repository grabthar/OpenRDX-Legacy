...
RDX.RegisterFeature({
	...
	title = i18n("Texture: Translation"); category = i18n("Animations");
	multiple = true;
...
if ]] .. usecdn .. [[ then 
	...
	UIFromDescriptor = function(desc, parent, state)
		...
		local owner = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Animation Group"), state, "AGTex_", nil);
...
RDX.RegisterFeature({
	...
	title = i18n("Texture: Rotation"); category = i18n("Animations");
	multiple = true;
	IsPossible = function(state)
		if not state:Slot("UnitFrame") then return nil; end
		if not state:Slot("Base") then return nil; end
...
if ]] .. usecdn .. [[ then 
	...
	UIFromDescriptor = function(desc, parent, state)
		...
		local owner = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Animation Group"), state, "AGTex_");
		if desc and desc.owner then owner:SetSelection(desc.owner); end
...
RDX.RegisterFeature({
	...
	title = i18n("Texture: Scale"); category = i18n("Animations");
	multiple = true;
	IsPossible = function(state)
		if not state:Slot("UnitFrame") then return nil; end
		if not state:Slot("Base") then return nil; end
...
RDX.RegisterFeature({
	...
	title = i18n("Texture: Alpha"); category = i18n("Animations");
	multiple = true;
	IsPossible = function(state)
		if not state:Slot("UnitFrame") then return nil; end
		if not state:Slot("Base") then return nil; end
