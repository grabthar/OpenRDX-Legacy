...
RDX.RegisterFeature({
	...
	title = i18n("AG Frame"); category = i18n("Animations");
	multiple = true;
	...
	ExposeFeature = function(desc, state, errs)
		...
		if state:Slot("AGFra_" .. desc.owner) then
			VFL.AddError(errs, i18n("Error AG already created:'") .. desc.owner .. "'.");
			return nil;
		else
...
if ]] .. usecdn .. [[ then 
	...
	UIFromDescriptor = function(desc, parent, state)
		...
		local er = RDXUI.EmbedRight(ui, i18n("Animation Loop Type"));
...
RDX.RegisterFeature({
	...
	title = i18n("AG Texture"); category = i18n("Animations");
	multiple = true;
	IsPossible = function(state)
		if not state:Slot("UnitFrame") then return nil; end
		if not state:Slot("Base") then return nil; end
...
if ]] .. usecdn .. [[ then 
	...
	UIFromDescriptor = function(desc, parent, state)
		...
		local owner = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Owner Texture"), state, "Texture_", nil);
		if desc and desc.owner then owner:SetSelection(desc.owner); end
...
RDX.RegisterFeature({
	...
	title = i18n("AG Text"); category = i18n("Animations");
	multiple = true;
	IsPossible = function(state)
		if not state:Slot("UnitFrame") then return nil; end
		if not state:Slot("Base") then return nil; end
	...
	ExposeFeature = function(desc, state, errs)
		...
		if not (desc.owner) or (not state:Slot("Text_" .. desc.owner)) then
			VFL.AddError(errs, i18n("Invalid text")); return nil;
		end
...
if ]] .. usecdn .. [[ then 
	...
	UIFromDescriptor = function(desc, parent, state)
		...
		local owner = RDXUI.MakeSlotSelectorDropdown(ui, i18n("Owner Text"), state, "Text_", nil);
		if desc and desc.owner then owner:SetSelection(desc.owner); end
