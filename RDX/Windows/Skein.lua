...
skeinLayer:SetParent(UIParent); skeinLayer:SetAllPoints(UIParent);
...
--	RDX.print("Creating skein from " .. tostring(srcw._path) .. " to " .. tostring(dstw._path));
...
function RDX.SkeinInstance:Weave(index, f1, f2)
	...
	local f1_l, f1_t, f1_r, f1_b = GetUniversalBoundary(f1);
	local f2_l, f2_t, f2_r, f2_b = GetUniversalBoundary(f2);
	if (not f1_l) or (not f2_l) then return; end
	VFLG.SetClipFlag(false);
	-- Case 1 (left F1 -> right F2)
	if(f1_l > f2_r) then
		self:UniversalLine(index, f1_l, (f1_t+f1_b)/2, f2_r, (f2_t+f2_b)/2, lerp5( (index-1)/self.order, self.w1, self.w2, self.c1.r, self.c2.r, self.c1.g, self.c2.g, self.c1.b, self.c2.b, self.c1.a, self.c2.a ) );
	else -- Case 2 (right F1 -> left F2)
		self:UniversalLine(index, f1_r, (f1_t+f1_b)/2, f2_l, (f2_t+f2_b)/2, lerp5( (index-1)/self.order, self.w1, self.w2, self.c1.r, self.c2.r, self.c1.g, self.c2.g, self.c1.b, self.c2.b, self.c1.a, self.c2.a ) );
	end
end
...
RDX.RegisterFeature({
	...
	ExposeFeature = function(desc, state, errs)
		...
		if not desc.tw then
			VFL.AddError(errs, i18n("Missing target window."));
			return nil;
		end
		if not desc.order then
			VFL.AddError(errs, i18n("Missing skein order."));
			return nil;
		end
		if (not desc.thick1) or (not desc.thick2) or (not desc.color) or (not desc.fadeColor) then
			VFL.AddError(errs, i18n("Missing visual parameters."));
			return nil;
		end
		return true;
	end;
	...
	UIFromDescriptor = function(desc, parent, state)
		...
		thick1:SetText(i18n("Starting thickness"));
		...
		thick2:SetText(i18n("Ending thickness"));
		if desc and desc.thick2 then thick2.editBox:SetText(desc.thick2); end
		ui:InsertFrame(thick2);

		local er = RDXUI.EmbedRight(ui, i18n("Starting color:"));
		local swatch_c = VFLUI.ColorSwatch:new(er);
		swatch_c:Show();
		if desc and desc.color then swatch_c:SetColor(explodeRGBA(desc.color)); end
		er:EmbedChild(swatch_c); er:Show();
		ui:InsertFrame(er);

		er = RDXUI.EmbedRight(ui, i18n("Ending color:"));
		local swatch_fc = VFLUI.ColorSwatch:new(er);
		swatch_fc:Show();
		if desc and desc.fadeColor then swatch_fc:SetColor(explodeRGBA(desc.fadeColor)); end
...
RDX.RegisterFeature({
	...
	ExposeFeature = function(desc, state, errs)
		...
		if desc.sort then
			...
			if not si then
				VFL.AddError(errs, i18n("Invalid sort pointer."));
				return nil;
			end
		else
		...
		if not desc.order then
			VFL.AddError(errs, i18n("Missing order."));
			return nil;
		end
	...
	ApplyFeature = function(desc, state)
		...
		state:Attach("CellPostPaintAdvice", true, function(w, cell, index, icv, uid, rdxUnit)
			...
			local th,w,r,g,b,a = lerp6((ix-1)/order, t1, t2, w1, w2, c1.r, c2.r, c1.g, c2.g, c1.b, c2.b, c1.a, c2.a);
	...
	UIFromDescriptor = function(desc, parent, state)
		...
		local order = VFLUI.LabeledEdit:new(ui, 50); order:Show();
		order:SetText(i18n("Size"));
		if desc and desc.order then order.editBox:SetText(desc.order); end
		ui:InsertFrame(order);

	  local orientation = VFLUI.RadioGroup:new(ui);
  	orientation:SetLayout(2,2);
	  orientation.buttons[1]:SetText(i18n("Orient Left"));
		orientation.buttons[2]:SetText(i18n("Orient Right"));
		...
		er = RDXUI.EmbedRight(ui, i18n("End color"));
		local swatch_fc = VFLUI.ColorSwatch:new(er);
		swatch_fc:Show();
		if desc and desc.fadeColor then swatch_fc:SetColor(explodeRGBA(desc.fadeColor)); end
		er:EmbedChild(swatch_fc); er:Show();
