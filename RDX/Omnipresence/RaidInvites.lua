...
function RDXI_Invite()
	...
	if (GetNumRaidMembers() > 0) and (IsRaidLeader() or IsRaidOfficer()) then
		...
		VFL.ZMSchedule(10, function() RDXI_BeginInvites(); end);
		return;
	end;
	...
	if (GetNumRaidMembers() == 0 and GetNumPartyMembers() == 0) then
		VFL.ZMSchedule(5, function() RDXI_CreateSoloRaid(); end);
		return;
	end;	
end;
...
function RDXI_CreateSoloRaid()
	...
	VFL.ZMSchedule(2, function() ConvertToRaid(); end);
	VFL.ZMSchedule(5, function() RDXI_VerifyRaid(); end);
end;
...
function Logistics.ScheduleScanPromote()
	if IsRaidLeader() then
		if not PromoteScanLockout then
			...
			VFL.ZMSchedule(2, function() ScanPromote(); end);
			VFL.ZMSchedule(5, function() PromoteScanLockout = false; end);
		end;
	end;
end;
